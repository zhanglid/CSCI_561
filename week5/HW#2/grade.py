...
for input_file_name in input_list:
    time_used = timeout_command(['python', 'hw2cs561s17.py', '-i', 'testcases2/' + input_file_name], TIME_OUT)
	...
	for line, right_line in zip(output.splitlines(), right_output.splitlines()):
            if line != right_line:
                print 'My output: ' + line
                print 'Right output: ' + right_line 
                break
        ...
        print input_file_name + ' pass in ' + str(time_used) + 's'

print 'pass/total: ' + str(total_pass) + '/' + str(len(input_list)) + ', avg time: ' + str(float(timeusedsum)/total_pass) + 's'
